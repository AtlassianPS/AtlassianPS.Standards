...
The action creates `## v1.2.3 - YYYY-MM-DD` immediately after `## Unreleased`, moves any existing Unreleased body plus valid `.changelog/*.md` fragment contents into that section, preserves the first-seen order of existing standard and custom sections, groups typed fragments under standard release-note headings, consolidates duplicate standard headings at their first position, appends fragment-only headings in standard order, and deletes only the consumed fragments. Fragment files should contain list items without their own `###` heading because the filename type supplies the heading.
